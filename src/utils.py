...
def analyser_manquants(df):
    """Détecte et affiche les valeurs manquantes"""
    print("\n" + "=" * 50)
...
def analyser_aberrantes(df):
    ...
    print("\n" + "=" * 50)
    print("⚠️  STATISTIQUES (chercher min/max suspects)")
    print("=" * 50)
...
def analyser_uniques(df):
    ...
    print("\n" + "=" * 50)
    print("🗑️  COLONNES POTENTIELLEMENT INUTILES")
    print("=" * 50)
    ...
    print(f"\n⚠️  Colonnes avec beaucoup de valeurs uniques :")
...
def analyser_echelles(df):
    ...
    print("\n⚠️  Les colonnes avec un grand écart nécessitent une normalisation !")
...
def analyser_cible(df, cible='Churn'):
    ...
    if pct.min() < 20:
        print("\n⚠️  Classes déséquilibrées ! Penser au rééquilibrage.")
    else:
...
def rapport_complet(df):
    """Lance toute l'analyse d'un coup"""
    analyser_shape(df)
    ...
    analyser_cible(df)
    analyser_aberrantes(df)  # en dernier car affiche des graphiques
